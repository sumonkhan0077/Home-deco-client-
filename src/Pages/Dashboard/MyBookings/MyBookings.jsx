  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/dashboard/my-bookings?email=${user.email}&sort=${type}&limit=${limit}&skip=${skip}`
      );
      setTotalBooking(res.data.totalBooking);
      setTotalPage(Math.ceil(res.data.totalBooking / limit));
      ...
      <div className="flex justify-between mx-6 my-6">
        <AnimatedSection variants="fadeUp">
          <h2 className="text-3xl text-primary">
            {" "}
            My Bookings Items{" "}
            <span className="font-bold">({totalBooking})</span>{" "}
          </h2>
        </AnimatedSection>
        {/* type sort */}
        <AnimatedSection variants="fadeUp">
                  ...
                  <tr key={service._id} className="bg-secondary text-white ">
                    ...
                    <td className="font-normal flex flex-col lg:flex-row gap-2">
                      ...
                      <Link
                        ...
                        to={`/services/${service.servicesId}`}
                      >
                    ...
                    >
                      {service?.paymentStatus === "paid" ? (
                        ...
                        <button
                          ...
                          className="main-btn2 bg-primary cursor-pointer text-md"
                        >
                          Pay
                        </button>
                      )}
